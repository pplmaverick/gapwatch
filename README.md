...
9. **Compliance filtering excludes transactions up front; Robinhood's specific use of it is inference.** The `extraPreTxFilter` / `extraPostTxFilter` hooks revert a filtered transaction out of the block entirely, leaving no receipt or log — this is confirmed against Nitro source, as is the existence of the `ArbFilteredTransactionsManager` precompile. That Robinhood uses those hooks for stock-token compliance filtering specifically is a reasonable inference from a private fork, **not** a verified fact.
