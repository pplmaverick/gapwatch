...
17. **GapwatchRegistryV2 and MockLendingPool were redeployed to mainnet on 2026-09-19 to add the filter-check + multiplier cross-validation logic (#14–#16), and this is a genuinely new contract, not an upgrade of the old one.** No proxy pattern is used anywhere in this project, so adding that logic to `recordVerification()` required a fresh `CREATE` deployment rather than an in-place upgrade — the new address (`0x88395FE9Ce32494C9a054F794bE6EbaE5Ec9203d`) shares the exact same constructor arguments as the previous one (`0x556e1cA65a003b1ffce58eDd14CE3c0c6F520137` — node set, `consensusVerifier`, `requiredBond`, `challengeWindow` all identical, confirmed by independently calling each getter on-chain, not assumed from `deployment.json`), but is a completely separate contract with its own, separate storage. Practical consequence: the one real consensus record this project has ever written (`0x11255751af281f9179a5b19dfecfdf53a6d377d700fffd00b00531517d38d369`, recorded 2026-09-16, see #13) **stays on the previous address permanently** — it was never, and will never be, copied or migrated to the new one. The new address started with zero records (confirmed via a full `eth_getLogs` scan immediately after deployment); it now has its first, [`0x17039804fb90dd5f6633080225bc2ff0ad080df6854377b83909b5d5da317697`](https://robinhoodchain.blockscout.com/tx/0x17039804fb90dd5f6633080225bc2ff0ad080df6854377b83909b5d5da317697) — a manual replay of the same real NVDA event on 2026-09-18, run through the pre-flight/sign/submit checklist (`docs/recordVerification-checklist.md`): `claimedFiltered`/`claimedMultiplier` were freshly re-verified against `isTransactionFiltered()`/`uiMultiplier()` immediately before signing rather than reused from the original 2026-09-16 submission, and both node signatures were locally recovered against the signed digest and confirmed to match the real node addresses before broadcast. `getVerification()` read back afterward matches exactly (`oldMultiplier=1000000000000000000`, `newMultiplier=1000775159164630595`, `wasFiltered=false`, same `referenceModelHash` as the original record on the previous address — not a placeholder). `MockLendingPool` was redeployed alongside `GapwatchRegistryV2` purely because its `registry` field is `immutable` with no setter (checked in `contracts/src/MockLendingPool.sol`) — there was no way to repoint the old pool at the new registry without redeploying the pool too. Both previous-deployment addresses remain listed above, not deleted, and both are still live, queryable contracts on mainnet — "previous" describes what this project points to, not that the address stopped existing or was emptied.
18. **`reference_model.py`'s hash formula changed on 2026-09-20 (`fb85e08`) to stop sealing a SQLite row id.** `verify_event()` used to include `event_id` — a per-database `AUTOINCREMENT` primary key — in the record it hashes. Two independent `events.db` files that backfilled the same real event in a different row order (the local dev machine had a gap at id 2 that the VPS's copy didn't) therefore sealed two different, non-reconcilable hashes for identical on-chain facts, defeating the point of an independently-reproducible verification. The fix replaces `event_id` with `event["tx_hash"]` — the one identifier intrinsic to the event itself, not to which database file it happens to live in. On 2026-09-20, all 7 rows (NVDA + the six newly-discovered tokens) were recomputed with the new formula and their `reference_model_hash` overwritten, on both the local dev database and the VPS's `/root/gapwatch-data/events.db` (via `docker exec` into the running `gapwatch-api` container, against the volume-mounted file — snapshotted with `md5sum` before and after, confirming only the 7 `reference_model_hash` values changed and every other column was untouched). Both databases now produce identical hashes per token, as intended. **NVDA's mainnet record is unaffected and intentionally not reconciled**: the one real `recordVerification()` call on `GapwatchRegistryV2` (see #17) sealed `referenceModelHash = 0x8ae207fbfb36087dc227455a640727cdb1e4be5760488ad1ee2e7e20fabe80ab`, computed under the *old* formula — that on-chain record is immutable once written and cannot be retroactively updated, so it permanently reflects the pre-fix formula while every local recomputation going forward uses the new one. None of the six new tokens had been submitted to any registry when this fix landed, so no on-chain state anywhere was affected by the change.
...
**Token registry: factory-event discovery, not `eth_getLogs` over an unbounded scope**
The token registry used to scan every address on chain for `UIMultiplierUpdated`, treating anything that ever emitted one as a token — indirect, and it never carried symbol/name. It now scans a single address, the token factory (`0x4783c67b63de2b358ac5951a7d41f47a38f3c046`), for its `TokenDeployed`-shaped event, which carries `(address, name, symbol)` directly. Verified against a full-history scan (block 0 → latest, confirmed this RPC has no `eth_getLogs` block-range limit): all 22 previously-known tokens are covered by the factory's 204 total deployments, so the switch lost no coverage. The registry persists a `last_scanned_factory_block` checkpoint and scans only the incremental range on later calls; on failure the checkpoint is deliberately not advanced, so the next run retries from the same point rather than skipping a gap. Scoping to one address is what makes a single-shot full-history scan practical here — an unscoped, address-agnostic scan across the whole chain (the old approach) is a different, much heavier query and was not re-tested at that scope after this change.
...
- Token registry (2026-09-18): auto-discovery via the token factory's deployment events replaces manually maintaining the token list. A reported symbol is trusted for monitoring only if it looks like a real ticker (1-6 uppercase letters/digits, one optional `.X`/`.XX` suffix) and contains no `DEFI`/`TEST`/`MOCK`/`DEMO`/`DUMMY`/`SAMPLE` keyword or underscore; anything that fails goes to a `pending_review` bucket instead — held, not monitored, and not silently dropped. `refresh_registry()` is wired into `state_machine_loop`'s existing 15s tick and mutates the running `FilterEngine`'s registry in place, so a token deployed while the process is already running is picked up without a container restart. Known limitation: the safety filter is a fixed allowlist pattern, not a check against Robinhood's own list of real tickers — a legitimately-named but non-stock deployment through the same factory would pass it, and a real ticker that doesn't fit the pattern (longer than 6 characters, or with punctuation other than one dot) would be wrongly held in `pending_review`.
